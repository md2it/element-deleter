...
- **اختصار البادئة داخل الصفحة** (`Ctrl+Shift+X` → `D`، Mac: `Cmd+Shift+X` → `D`) يعمل في الصفحة ويتطلب حقن content script مسبقًا. بدون صلاحية مضيف دائمة لا يمكن أن يكون أول إجراء في تبويب جديد. البدائل دون inject مسبق: أيقونة شريط الأدوات، الأمر `_execute_action`، أو `activate-deactivate` في إعدادات اختصارات المتصفح. يعمل Esc / التراجع في وضع الحذف داخل الصفحة بعد الحقن؛ ويُنفَّذ الأمران `deactivate-delete-mode` و`undo-delete` من الخلفية.
- **الحذف من قائمة السياق في Chrome** لعنصر DOM عام (ليس image/link/editable) قد يفعّل وضع الحذف بدل الحذف الفوري إذا لم يكن السكربت على الصفحة بعد. لا يوفر Chrome `menus.getTargetElement` ولا `contextMenus.onShown` مع `activeTab` قبل اختيار البند. يحدد Firefox العنصر عبر `menus.getTargetElement`. بعد وجود السكربت على الصفحة يحذف قائمة سياق Chrome أيضًا العناصر العامة فورًا.
